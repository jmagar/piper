...
export async function GET() {
  ...
  try {
    ...
    appLogger.http.error('Error reading or parsing config file:', error as Error);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      appLogger.http.info('[MCP Config API] Config saved successfully. Checking for new servers to initialize...');
      // const { checkAndInitializeNewServers } = await import('../../../lib/mcp/mcpManager'); // MCPManager deleted
      // await checkAndInitializeNewServers(); // MCPManager deleted
      appLogger.http.info('[MCP Config API] New server initialization check completed.');
    } catch (initError: unknown) {
      appLogger.http.error('[MCP Config API] Error during new server initialization:', initError instanceof Error ? initError : new Error(String(initError)));
    ...
    appLogger.http.error('Invalid request body:', error as Error);
