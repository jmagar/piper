...
import { DialogEditPromptTrigger } from "./dialog-edit-prompt/dialog-trigger-edit-prompt"
...
type PromptDetailProps = {
  ...
  onPromptClickAction?: (promptId: string) => void
  morePrompts?: Array<{
    id: string
    name: string
    description: string
...
export function PromptDetail({
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText(`${window.location.origin}/prompts/${slug}`)
    setCopied(true)
    setTimeout(() => setCopied(false), 1000)
  }

  useEffect(() => {
    if (morePrompts.length > 0 && isFullPage) {
      morePrompts.forEach((prompt) => {
        router.prefetch(`/prompts/${prompt.slug}`)
      })
    }
  }, [morePrompts, router, isFullPage])

  const handlePromptClick = (prompt: { id: string; slug: string }) => {
    if (onPromptClickAction) {
      onPromptClickAction(prompt.id)
    } else {
  ...
  const handleDelete = async () => {
    setIsDeleting(true)
    try {
      const response = await fetchClient(`/api/delete-prompt/${id}`, {
        method: "DELETE",
      ...
      if (!isFullPage && onPromptClickAction) {
        onPromptClickAction("")
      }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {morePrompts && morePrompts.length > 0 && (
              ...
              {morePrompts.map((prompt, index) => (
                <div
                  ...
                  className={cn(
                    ...
                    index === morePrompts.length - 1 && "mr-6"
                  )}
                >
