"use client"

import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { Textarea } from "@/components/ui/textarea"
import { AlertCircle, X } from "lucide-react"
import type React from "react"

type RuleFormData = {
  name: string
  description: string
  systemPrompt: string
}

type EditRuleFormProps = {
  formData: RuleFormData
  error: { [key: string]: string }
  isLoading: boolean
  handleInputChangeAction: (
    e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>
  ) => void
  handleSubmitAction: (e: React.FormEvent) => Promise<void>
  onCloseAction: () => void
  isDrawer?: boolean
}

export function EditRuleForm({
  formData,
  error,
  isLoading,
  handleInputChangeAction,
  handleSubmitAction,
  onCloseAction,
  isDrawer = false,
}: EditRuleFormProps) {
  return (
    <div
      className={`space-y-0 ${isDrawer ? "p-0 pb-16" : "py-0"} overflow-y-auto`}
    >
      {isDrawer && (
        <div className="border-border mb-2 flex items-center justify-between border-b px-4 pb-2">
          <h2 className="text-lg font-medium">Edit rule</h2>
          <Button variant="ghost" size="icon" onClick={onCloseAction}>
            <X className="h-4 w-4" />
          </Button>
        </div>
      )}

      <div className="px-6 py-4">
        <div className="bg-muted/50 mb-6 rounded-lg p-3">
          <p className="text-sm">
            Update your rule&apos;s name, description, and prompt content.
            Changes will be reflected immediately when the rule is @mentioned.
          </p>
        </div>

        <form onSubmit={handleSubmitAction} className="space-y-6">
          {/* Rule Name */}
          <div className="space-y-2">
            <Label htmlFor="name">Rule name</Label>
            <Input
              id="name"
              name="name"
              placeholder="My Rule"
              value={formData.name}
              onChange={handleInputChangeAction}
              className={error.name ? "border-red-500" : ""}
            />
            <p className="text-muted-foreground text-xs">
              Used for @mentions (e.g., @my-rule)
            </p>
            {error.name && (
              <div className="mt-1 flex items-center text-sm text-red-500">
                <AlertCircle className="mr-1 h-4 w-4" />
                <span>{error.name}</span>
              </div>
            )}
          </div>

          {/* Description */}
          <div className="space-y-2">
            <Label htmlFor="description">Description</Label>
            <Input
              id="description"
              name="description"
              placeholder="A short description of what this rule does"
              value={formData.description}
              onChange={handleInputChangeAction}
              className={error.description ? "border-red-500" : ""}
            />
            <p className="text-muted-foreground text-xs">
              Brief explanation visible in rule listings
            </p>
            {error.description && (
              <div className="mt-1 flex items-center text-sm text-red-500">
                <AlertCircle className="mr-1 h-4 w-4" />
                <span>{error.description}</span>
              </div>
            )}
          </div>

          {/* System Prompt */}
          <div className="space-y-2">
            <Label htmlFor="systemPrompt">Prompt content</Label>
            <Textarea
              id="systemPrompt"
              name="systemPrompt"
              placeholder="You are a helpful assistant that specializes in..."
              value={formData.systemPrompt}
              onChange={handleInputChangeAction}
              className={`h-32 font-mono ${error.systemPrompt ? "border-red-500" : ""}`}
            />
            <p className="text-muted-foreground text-xs">
              The prompt text that will be added when this rule is mentioned
            </p>
            {error.systemPrompt && (
              <div className="mt-1 flex items-center text-sm text-red-500">
                <AlertCircle className="mr-1 h-4 w-4" />
                <span>{error.systemPrompt}</span>
              </div>
            )}
          </div>

          {/* Submit Button */}
          <Button type="submit" className="w-full" disabled={isLoading}>
            {isLoading ? "Updating Rule..." : "Update Rule"}
          </Button>
        </form>
      </div>
    </div>
  )
} 