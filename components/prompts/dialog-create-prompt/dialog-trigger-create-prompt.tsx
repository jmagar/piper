...
type DialogCreatePromptTriggerProps = {
  trigger: React.ReactNode
}

export function DialogCreatePromptTrigger({
  trigger,
}: DialogCreatePromptTriggerProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const apiResponse = await fetchClient("/api/create-prompt", {
        method: "POST",
      ...
      toast({ title: "Prompt created successfully!", status: "success" })
      setOpen(false)
      ...
      toast({ title: "Error creating prompt", description: errorMessage, status: "error" })
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <Drawer open={open} onOpenChange={setOpen}>
        ...
        <DrawerContent className="max-h-[90vh]">
          <VaulDrawerHeader className="text-left">
            <VaulDrawerTitle>Create prompt</VaulDrawerTitle>
