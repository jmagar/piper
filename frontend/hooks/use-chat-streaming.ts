...
export function useChatStreaming() {
  ...
  const detectToolInvocation = React.useCallback((content: string): boolean => {
    ...
    const toolPatterns = [
      ...
      /I'll (search|read|check|look at|examine|analyze|run|execute|invoke|fetch|get|retrieve)/i,
      /<function_calls>/i,
      /<invoke name=/i,
      /<function_calls>/i,
      /<invoke/i,
      /I need to (search|read|check|look at|examine|analyze|run|execute|invoke|fetch|get|retrieve)/i,
      /Let me (search|read|check|look at|examine|analyze|run|execute|invoke|fetch|get|retrieve)/i,
      /I will (search|read|check|look at|examine|analyze|run|execute|invoke|fetch|get|retrieve)/i,
      /Using the ([a-zA-Z_]+) tool/i,
      /Invoking ([a-zA-Z_]+) to/i,
      /\*\*Invoking Tool\*\*/i,
      /Calling the ([a-zA-Z_]+) tool/i,
      /Let me check the (current|time|date|weather|temperature|stock|price)/i,
      /current time in ([A-Za-z\s]+) is/i,
      /([A-Za-z\s]+) timezone/i,
      /time is ([0-9]{1,2}:[0-9]{2})/i,
      /date is ([A-Za-z]+ [0-9]{1,2})/i
    ];
