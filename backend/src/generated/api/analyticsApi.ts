...
import { GetMetrics200Response } from '../models/getMetrics200Response';
...
export class AnalyticsApi {
     ...
     * @param userId 
     */
    public async getMetrics (startDate?: Date, endDate?: Date, userId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: GetMetrics200Response;  }> {
        const localVarPath = this.basePath + '/api/analytics/metrics';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: GetMetrics200Response;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "GetMetrics200Response");
