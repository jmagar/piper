...
export function loadConfig(path: string): Config {
  try {
    log('Loading configuration from %s', path);
    ...
    error('Failed to load configuration from %s: %s', path, err instanceof Error ? err.message : String(err));
    if (err instanceof Error) {
      throw new Error(`Failed to load configuration from "${path}": ${err.message}`);
    }
    throw err;
  }
}
